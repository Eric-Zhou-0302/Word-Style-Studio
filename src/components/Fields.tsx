...
export function Field({label,children,hint,onReset,inherited=false,inheritanceSource,className=''}:{label:string;children:ReactNode;hint?:string;onReset?:()=>void;inherited?:boolean;inheritanceSource?:string;className?:string}) {
  const documentDefault=inheritanceSource==='文档默认值';
  const resetLabel=`重置${label}为${documentDefault?'文档默认值':`继承值${inheritanceSource?`（${inheritanceSource}）`:''}`}`;
  return <div className={`field ${className}`}><div className="field-heading"><span>{label}</span>{onReset?<button className="reset-field" type="button" title={resetLabel} aria-label={resetLabel} onClick={onReset}><RotateCcw size={12}/></button>:inherited?<span className="inherit-label" title={documentDefault?'使用文档默认值':inheritanceSource?`继承自${inheritanceSource}`:undefined}>{documentDefault?'文档默认':'继承'}</span>:null}</div>{children}{hint&&<span className="field-hint">{hint}</span>}</div>;
}
...
export function ColorInput({label,value,onChange}:{label:string;value:string;onChange:(v:string)=>void}){
  ...
  const commonColors=[['黑色','#000000'],['深灰','#595959'],['灰色','#A6A6A6'],['浅灰','#D9D9D9'],['白色','#FFFFFF'],['红色','#C00000'],['橙色','#ED7D31'],['黄色','#FFC000'],['绿色','#548235'],['青色','#008C95'],['蓝色','#0070C0'],['深蓝','#203864'],['紫色','#7030A0']] as const;
  function choose(color:string){setDraft(color);onChange(color);setOpen(false);trigger.current?.focus();}
  return <div className="color-picker" ref={root} onKeyDown={e=>{if(e.key==='Escape'&&open){e.stopPropagation();setOpen(false);trigger.current?.focus();}}} onBlur={e=>{if(!e.currentTarget.contains(e.relatedTarget))setOpen(false);}}><div className="color-input"><button ref={trigger} type="button" className="color-menu-trigger" aria-label={`${label}选择颜色`} aria-expanded={open} aria-controls={paletteId} onClick={()=>setOpen(v=>!v)}><span style={{backgroundColor:value}}/><ChevronDown size={12}/></button><input aria-label={label} value={draft} maxLength={7} onChange={e=>{setDraft(e.target.value);if(/^#[0-9a-f]{6}$/i.test(e.target.value))onChange(e.target.value);}} onBlur={()=>setDraft(value)}/></div><input ref={picker} className="native-color-picker" type="color" aria-label={`${label}更多颜色色板`} tabIndex={-1} value={value} onChange={e=>{setDraft(e.target.value);onChange(e.target.value);}}/>{open&&<div className="color-popover" id={paletteId} role="group" aria-label={`${label}常用颜色`}><strong>常用颜色</strong><div className="common-colors">{commonColors.map(([name,color])=><button key={color} type="button" className="color-swatch" style={{backgroundColor:color}} title={`${name} ${color}`} aria-label={`${label}：${name} ${color}`} aria-pressed={value.toUpperCase()===color} onClick={()=>choose(color)}/>)}</div><button type="button" className="more-colors" onClick={()=>{setOpen(false);trigger.current?.focus();picker.current?.click();}}>更多颜色…</button></div>}</div>;
}
