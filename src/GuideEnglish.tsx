...
export default function GuideEnglish() {
 ...
 <ol>
 <li><strong>Open the workspace and choose a starting point.</strong> Continue a saved project, start from the example, or start blank. In the example, select “Report Body” to adjust fonts and paragraphs; its report styles are independent of one another. A blank project begins with Normal.</li>
 <li><strong>Set up your headings.</strong> Select “Heading 1,” “Heading 2,” and so on to set font sizes, paragraph spacing, and page-break rules. You can also create a style in the left sidebar.</li>
 <li><strong>Configure your lists.</strong> Use a multilevel list for chapters, a numbered list for steps, and bullets for unordered items. The example’s “Heading Numbering” already links its first six levels to the corresponding headings; “Body Numbering” has two levels, with a separate “Body Bullets” list.</li>
 ...
 <p>The interface language applies to the home page, workspace, and this guide, and is remembered in this browser. New example and blank projects use names in the current interface language. Switching languages, restoring a saved project, or importing a file preserves existing project, style, and list names, including names saved in Chinese. The names below match a new example created in English.</p>
 </Section>
 <Section id="concepts" title="02 · Understand the essentials">
 <h3>A style gives a set of formatting a name</h3>
 <p>For example, “Report Body” can mean SimSun, 12 pt, and 1.5-line spacing, while “Heading 1” can mean SimHei, 20 pt, with space above. Styles let you change similar content together. Formatting selected text directly, such as making it bold or changing its color, can override the style. For Word’s own controls, see <a href="https://support.microsoft.com/en-us/word/customize-or-create-new-styles" target="_blank" rel="noreferrer">Microsoft’s guide to styles</a>.</p>
 ...
 <p><strong>Based on</strong> identifies the source of inherited formatting. If a heading is based on a body style, any property you have not set explicitly follows that body style. Once you set a font size for the heading, changing the body’s size no longer changes it. Use the reset button beside a field to restore inheritance. Without a base style, fields show “Document default”; switches show their effective on/off value, and resetting restores the document default. With a base style, the field shows “Inherited,” and its tooltip identifies the source. <strong>Style for following paragraph</strong> determines the style used after you press Enter in Word. The example’s report headings return to “Report Body.”</p>
 ...
 <p>On a first visit to the workspace, or when you create a project from the example, the app loads 29 independent report styles without Normal. In English, this project is named “General Report Styles.” None is based on another style. They cover the cover title, contents entries, six heading levels, report body, figure and table captions, sources, footnotes, and table text. “Standard Table” is also independent and controls borders, cell padding, and related properties. Separate paragraph styles format header, body, and footer text within cells.</p>
 ...
 ['Create blank project (project menu)', 'Replaces it with “Untitled Project,” containing only Normal and no lists when created in English. Base fonts: SimSun 12 pt and Times New Roman for Latin text; 1.5-line spacing; A4 portrait; 2.54 cm margins; header/footer distances of 1.27 cm'],
 ['Clear styles and lists (sidebar trash icon)', 'Removes all styles except Normal and all lists, keeping body formatting, the project name, and page settings. Creates a basic Normal style named in the current interface language if absent, and sets its following paragraph style to itself'],
 ...
 <ol><li>Resolve any “Needs attention” status, then choose “Export Word.”</li><li>Select a DOCX document or DOTX template and optionally enable “Include style samples.” Sample text follows the current interface language; your style names and numbering rules are preserved.</li><li>Open the download in desktop Word. By default, the body is blank, but style and list definitions are already in the file.</li><li>When writing, place the cursor in a paragraph and apply a paragraph or heading style. Apply character styles to selected text and table styles to tables. A style linked to numbering brings in its list formatting.</li></ol>
