...
export default function Guide({hash}:{hash:string}){
 ...
 <p>页面顶部可切换简体中文与 English，首页、工作区与使用文档共用语言选择，当前浏览器会记住设置。新建示例和空白方案时，方案、样式及列表名称使用当前界面语言；切换语言、恢复已保存方案或导入文件不会改动已有名称。英文示例沿用同一套中文报告的版式、字体和编号规则。</p>
 </Section>
 ...
 ['清空样式与列表（左侧顶部垃圾桶）','删除除 Normal 外的样式及全部列表，保留当前正文格式、方案名称和页面设置；若没有 Normal，则按当前界面语言创建“正文”或“Normal”。正文的后续段落恢复为自身'],
 ...
 <ol><li>确认顶部没有“待完善”，点击“导出 Word”。</li><li>选择 DOCX 文档或 DOTX 模板；按需要开启“包含样式示例”。示例文字使用当前界面语言，已有样式名称和编号规则保持原样。</li><li>下载后在桌面 Word 中打开检查。默认导出只保留空白正文，但样式和列表定义已在文件内。</li><li>写作时，把光标放入段落并应用相应段落/标题样式；字符样式应用于选中文字，表格样式应用于表格。关联编号的样式会带入列表格式。</li></ol>
